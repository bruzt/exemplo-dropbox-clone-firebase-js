...
class DropboxController {
    ...
    constructor () {
        ...
        this.inputFilesElement = document.getElementById("files");
        this.snackbarModalElement = document.getElementById("react-snackbar-root");
        this.progressbarElement = this.snackbarModalElement.querySelector(".mc-progress-bar-fg");
        this.fileNameElement = this.snackbarModalElement.querySelector(".filename");
        this.timeLeftElement = this.snackbarModalElement.querySelector(".timeleft");
        this.listFilesElement = document.getElementById("list-of-files-and-directories");

        this.onSelectionChange = new Event('selectionChange');

        this.btnSendFileElement = document.getElementById("btn-send-file");
        this.btnNewFolder = document.getElementById("btn-new-folder");
        this.btnRename = document.getElementById("btn-rename");
        this.btnDelete = document.getElementById("btn-delete");

        this.browseNavigationElement = document.getElementById("browse-location");
    ...
    initEvents(){
        ...
        this.btnNewFolder.addEventListener('click', (event) => {
            ...
            if( ! (newFolderName === null || newFolderName.match(/^ *$/) !== null)){
                ...
                //this.getFirebaseReference().push().child(newFolderName);
            }
        });
        ...
        this.btnDelete.addEventListener('click', async (event) => {
            ...
            try {
                ...
                responses = await this.removeItemsTask();
        ...
        this.listFilesElement.addEventListener('selectionChange', (event) => {
        ...
        this.inputFilesElement.addEventListener('change', async (event) => {
            ...
            try {
                ...
                responses = await this.uploadTask(event.target.files);
            ...
            responses.forEach( (response) => {
                ...
                this.getFirebaseReference(this.folderNow).push().set({      // (response.file['input-file']);
            ...
            promises.push(new Promise( async (resolve, reject) => {
                ...
                if(file.type === 'folder'){
                    ...
                    try {
                        ...
                        await this.removeFolder(this.currentFolder.join('/'), file.name);
                    ...
                    try {
                        ...
                        await this.removeFile(this.currentFolder.join('/'), file.name);
    ...
    removeFile(storageRef, fileName){

        let fileRef = firebase.storage().ref(storageRef).child(fileName);
    ...
    removeFolder(ref, name){
        ...
        return new Promise( (resolve, reject) => {
            ...
            let firebaseRef = this.getFirebaseReference(ref + '/' + name);       
                ...
                snapshot.forEach( async (item) => {
                    ...
                    if(data.type === 'folder'){
                        ...
                        try {
                            ...
                            await this.removeFolder(ref + '/' + name, data.name);

                            //await firebase().ref(ref + '/' + data.name).remove();
                        ...
                        try {
                            ...
                            await this.removeFile(ref + '/' + name, data.name);
    ...
    modalShow(show = true){
        ...
        if(show){
            ...
            this.snackbarModalElement.style.display = 'block'

        } else {

            this.snackbarModalElement.style.display = 'none'
        }
...
/*
    ...
    */
    ...
    uploadTask(files){
        ...
        this.folderNow = this.currentFolder.join('/');
        ...
        [...files].forEach( (file) => {
            ...
            promises.push(new Promise( (resolve, reject) => {
                ...
                let fileReference = firebase.storage().ref(this.folderNow).child(file.name);

                let task = fileReference.put(file);
                ...
                let success = async () => {
                    ...
                    try {
                        ...
                        let metadata = await fileReference.getMetadata();
                        metadata.downloadURLs = [await fileReference.getDownloadURL()];
            ...
            onprogress = (event) => this.uploadProgress(event, file);

            onloadstart = () => this.uploadStartTime = Date.now();
    ...
    uploadProgress(event, file){
        ...
        let timeSpent = Date.now() - this.uploadStartTime;
        ...
        this.fileNameElement.innerHTML = '"' + file.name + '"' + "<br>"; 

        this.timeLeftElement.innerHTML = percent + "%" + " - " + this.formatTimeLeft(timeleft);
        ...
        li.addEventListener('click', (event) => {
            ...
            if(event.shiftKey){
                ...
                if(firstLi) {
                    ...
                    let sortedIndex = [startIndex, endIndex].sort();

                    lis.forEach((element, index) => {

                        if(index >= sortedIndex[0] && index <= sortedIndex[1]){
            ...
            if(! event.ctrlKey){  /*&& ! li.classList.contains('selected')*/
                ...
                this.listFilesElement.querySelectorAll('li.selected').forEach((element) => {
    ...
    openFolder(){
        ...
        if(this.lastFolder) 
            this.getFirebaseReference(this.lastFolder).off('value');
    ...
    renderBrowseNavigation(){
        ...
        this.browseNavigationElement.innerHTML = nav.innerHTML;

        this.browseNavigationElement.querySelectorAll('a').forEach( (a) => {
        ...
        this.getFirebaseReference().on('value', (snapshot) => {
            ...
            snapshot.forEach( (item) => {
                ...
                if(data.type){
                    ...
                    this.listFilesElement.appendChild(this.getFileView(key, data));
